...
import { AgentProfile } from 'src/cms/entities/profile.schema';
import { CreateBaseProfileCommand } from '@app/contracts/commands/profile.command';
...
export class UsersService extends BaseService<User> {
  ...
  private readonly agentProfileModel: PaginateModel<AgentProfile>;
  constructor(
    ...
    agentProfileModel: PaginateModel<AgentProfile>,
  ...
  public async create(command: CreateUserCommand): Promise<User> {
    try {
      ...
      const hash = generateAgentHash(user._id.toHexString());
      const profile: CreateBaseProfileCommand = {
        fullName: command.firstName,
        npn: command.userName,
        hash: hash,
      };
      const agentProfileExisted = await this.agentProfileModel.findOne({
        npn: command.userName,
      });
      if (agentProfileExisted) {
        // updateProfile
        console.log('updating porofile', command.userName);
        ...
        try {
          const createdProfile = new this.agentProfileModel(profile);
  ...
  async getPhoneNumber(user: User): Promise<{ phone: string; id: string }> {
    const identity = await this.objectModel.findById({
      _id: user._id.toHexString(),
    });
    return { phone: identity?.phone, id: identity?._id.toHexString() };
  }
  async updatePhoneNumber(userName: string, phoneNumber: string) {
    const user = await this.objectModel.findOne({ userName });
    if (!user) {
  ...
  async getAdminUserByPhone(phone: string): Promise<User> {
    return await this.objectModel.findOne({
      $or: [{ phone }, { otherPhones: phone }],
  ...
  // find user by npn
  async findByNpn(npn: string): Promise<User> {
    return await this.objectModel.findOne({
      userName: npn,
    });
  }

  async updateByUsername(npn: string, data: any): Promise<User> {
    return await this.objectModel.findOneAndUpdate(
      {
        userName: npn,
      },
